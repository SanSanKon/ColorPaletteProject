...
currentHex.forEach(hex => {
    hex.addEventListener("click", () => {//arrow func is neccessary to invoke copyToClipboard and pass a hex as parameter to it
        copyToClipboard(hex);
    });
});
...
function randomColors() {
    initialColors = [];
    colorDivs.forEach((div, index) => {
        const hexText = div.children[0]; //First child of div - 'h2'
...
function savedPalette(e) {
    ...
    const paletteObj = {name: name, colors: colors, nr: paletteNr} //name - is equal to saveInput.value
